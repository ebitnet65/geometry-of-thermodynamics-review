This is the README file for the LaTeX Article Template for Royal Society of Chemistry Journals.

The zip file contains:

 - rsc-articletemplate-pccp.tex (the template with example text)

 - rsc-articletemplate-pccp.pdf (an example of the template output PDF)

 - rsc.bib (example bibliography)

 - example1.pdf and example2.pdf (example figures)

 - head_foot (folder containing header and footer images)

 - rsc.bst (the RSC bibliography style file).


The following files need to be in the same folder as rsc-articletemplate-pccp.tex for the example template to compile correctly:

 - head_foot

 - example1.pdf

 - example2.pdf

 - rsc.bib

 - rsc.bst.

Please note: Supplementary Information (SI) files must be uploaded separately in the ScholarOne submission site. All other files should be uploaded to Overleaf and will be included in the source files ZIP file when transferred to ScholarOne. 

If you receive an error message from LaTeX that you are missing a necessary package, you need to add the package to your LaTeX installation. Instructions for installing extra packages can be found here: https://en.wikibooks.org/wiki/LaTeX/Installing_Extra_Packages (Please note that this is an external website that is not maintained by the Royal Society of Chemistry).

